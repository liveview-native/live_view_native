defmodule LiveViewNative.Component.Declarative do
  @moduledoc false
  ...
  def __on_definition__(env, kind, name, args, _guards, body) do
    ...
    cond do
      check? and length(args) == 2 and body == nil ->
      ...
      check? ->
        ...
        validate_misplaced_attrs!(attrs, env.file, fn ->
          case length(args) do
            2 ->
              "could not define attributes for function #{name}/2. " <>
                "Please make sure that you have `use LiveViewNative.Component` and that the function has no default arguments"

            arity ->
              "cannot declare attributes for function #{name}/#{arity}. Components must be functions with arity 2"
          end
        end)
        ...
        validate_misplaced_slots!(slots, env.file, fn ->
          case length(args) do
            2 ->
              "could not define slots for function #{name}/2. " <>
                "Components cannot be dynamically defined or have default arguments"

            arity ->
              "cannot declare slots for function #{name}/#{arity}. Components must be functions with arity 2"
          end
        end)
        ...
        def_body =
          if global_name do
            quote location: :keep do
              ...
              super(Phoenix.Component.assign(merged, unquote(global_name), globals), interface)
            end
          else
            quote do
              merged =
              ...
              super(merged, interface)
            end
          end

        merge =
          quote line: line do
            Kernel.unquote(kind)(unquote(name)(assigns, interface)) do
              unquote(def_body)
            end
          end
  ...
  defp raise_if_function_already_defined!(env, name, slots, attrs) do
    if Module.defines?(env.module, {name, 2}) do
      {:v1, _, meta, _} = Module.get_definition(env.module, {name, 2})
  ...
  defp ensure_used!(module, line, file) do
    if !Module.get_attribute(module, :__attrs__) do
      compile_error!(
        line,
        file,
        "you must `use LiveViewNative.Component` to declare attributes. It is currently only imported."
      )
    end
  end
end
