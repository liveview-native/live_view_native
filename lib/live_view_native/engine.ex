defmodule LiveViewNative.Engine do
  @moduledoc """
  The HTMLEngine that powers `ios.heex` templates. It is mostly
  inherited from `Phoenix.LiveView.HTMLEngine` with a few changes
  ...
  defdelegate __assigns__(assigns, key, parent_changed), to: Phoenix.LiveView.HTMLEngine
  defdelegate __inner_block__(do_block, name), to: Phoenix.LiveView.HTMLEngine
  defdelegate component(func, assigns, caller), to: Phoenix.LiveView.HTMLEngine
  ...
  defp push_tag(state, token) do
    # Void tags are not supported for LiveView Native targets by default,
    # so we always append tokens to the stack
